...
namespace BusinessLayer
{
    public class KyCongChiTiet
    {
		...
		{
			return db.KYCONGCHITIETs.FirstOrDefault(x => x.MAKYCONG == makycong && x.MANV == manv);
		}
		...
		public void phatSinhKyCongChiTiet(int thang, int nam)
		{
			...
			foreach (var item in lstNV)
			{
				...
				kycongchitiet.NGAYCONG = Myfunction.demSoNgayLamViec(thang,nam);
				kycongchitiet.TONGNGAYCONG = Myfunction.demSoNgayLamViec(thang, nam);
		...
		{
			var kycongchitiet = db.KYCONGCHITIETs.FirstOrDefault(x => x.MAKYCONG == kcct.MAKYCONG && x.MANV == kcct.MANV);
